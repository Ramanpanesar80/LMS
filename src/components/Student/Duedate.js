...
const Assignments = () => {
        ...
        (snapshot) => {
          const assignmentsData = snapshot.docs.map((doc) => {
            ...
            return {
              id: doc.id,
              ...data,
              dueDate: data.dueDate ? data.dueDate.toDate() : null, // Safely convert dueDate
            };
          });
  ...
  const handleEditAssignment = (assignment) => {
    if (!assignment.dueDate) {
      alert("This assignment does not have a valid due date.");
      return;
    }
    setEditingAssignment(assignment);
    setNewDueDate(assignment.dueDate.toISOString().split("T")[0]); // Pre-fill with current due date
  };
